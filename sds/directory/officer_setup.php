...
WHERE officerid=$number AND username='$username_esc' AND
      position='$position_esc' AND removed IS NULL
ENDQUERY;
	...
	pg_free_result($result);
      ...
      $query = <<<ENDQUERY
INSERT INTO officers (username,       position)
VALUES               ('$username_esc','$position_esc')
RETURNING officerid
ENDQUERY;
      $result = sdsQuery($query);
      if(!$result) {
...
$query = <<<ENDQUERY
SELECT officerid,position,username FROM officers
...
while($data = pg_fetch_object($result)) {
  ...
  echo "      Title:<input type='text' name='position[officer:",
    $data->officerid,
...
<div id='officertemplate' class='dragtemplate'>
  Title:<input type='text' name='position[]' />
